...
<main class="h-full overflow-y-auto">
    <div class="container px-6 mx-auto grid">
        ...
        <form
            action="{{route('transaksi-unit.update', $plant->id)}}"
            method="POST"
        ...
        >
            ...
            <label class="block mt-4 text-sm">
                ...
                <input
                    name='nom_unit'
                    value="{{ old('nom_unit', $plant->nom_unit) }}"
            ...
            <label class="block mt-4 text-sm">
                ...
                >
                    @foreach($model as $data)
                        <option value="{{$data->model}}" {{ old('model') == $data->model ? "selected" : "" }}>{{$data->model}}</option>
                    @endforeach
                </select>
            </label>
            ...
            <label class="block mt-4 text-sm">
                ...
                >
                    @foreach($type_unit as $tp)
                        <option value="{{$tp->type_unit}}" {{ old('type_unit') == $tp->type_unit ? "selected" : "" }}>{{$tp->type_unit}}</option>
                    @endforeach
                </select>
            </label>
            ...
            <label class="block mt-4 text-sm">
                ...
                >
                    @foreach($engine_brand as $data)
                        <option value="{{$data->engine_brand}}" {{ old('engine_brand') == $data->engine_brand ? "selected" : "" }}>{{$data->engine_brand}}</option>
                    @endforeach
                </select>
            </label>
            ...
            <label class="block mt-4 text-sm">
                ...
                <input
                    name="engine_model"
                    value="{{ old('engine_model', $plant->engine_model) }}"
                    class="block shadow-sm border p-2 rounded-md w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-stone-400 focus:outline-none focus:shadow-outline-stone dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
            ...
            <label class="block mt-4 text-sm">
                ...
                <input
                    name="engine_sn"
                    value="{{ old('engine_sn', $plant->engine_sn) }}"
                    class="block shadow-sm border p-2 rounded-md w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-stone-400 focus:outline-none focus:shadow-outline-stone dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
